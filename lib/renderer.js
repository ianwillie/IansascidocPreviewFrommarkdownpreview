...
var render =  function (text, filePath) {
  ...
  'attributes': {'allow-url-read': true, 'source-highlighter': 'rouge', 'standalone': true }}    ); // , WORKS
  ...
  //textToHTML = Asciidoctor.convert(text,   { 'safe': 'server',   'attributes': { 'showtitle': true }}); // ,  WORKS
   ...
   'attributes': { 'linkcss': false , 'icons': 'font'}});  // WORKS butdoes NOT save .html */
...
//   }
 ...
 html = createDOMPurify().sanitize(html, {
  ...
  //resolveImagePaths(fragment, filePath) // ianscomment seems to have no effect
