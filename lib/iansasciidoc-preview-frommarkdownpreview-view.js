...
const isAsciidoctor = atom.config.get("iansasciidoc-preview-frommarkdownpreview.isAsciidoctor");

module.exports = class IansasciidocPreviewFrommarkdownpreviewView {
  static deserialize (params) {
    return new IansasciidocPreviewFrommarkdownpreviewView(params)
  }
  ...
  constructor ({ editorId, filePath }) {
    ...
    this.element.classList.add('iansasciidoc-preview-frommarkdownpreview')
  ...
  serialize () {
    return {
      deserializer: 'IansasciidocPreviewFrommarkdownpreviewView',
  ...
  copy () {
    return new IansasciidocPreviewFrommarkdownpreviewView({
      editorId: this.editorId,
      filePath: this.getPath() != null ? this.getPath() : this.filePath
    })
  }
  ...
  handleEvents () {
    ...
    atom.commands.add(this.element, {
      ...
      'iansasciidoc-preview-frommarkdownpreview:select-all': () => {
        this.selectAll()
      },
      'iansasciidoc-preview-frommarkdownpreview:zoom-in': () => {
        const zoomLevel = parseFloat(getComputedStyle(this.element).zoom)
        this.element.style.zoom = zoomLevel + 0.1
      },
      'iansasciidoc-preview-frommarkdownpreview:zoom-out': () => {
        const zoomLevel = parseFloat(getComputedStyle(this.element).zoom)
        this.element.style.zoom = zoomLevel - 0.1
      },
      'iansasciidoc-preview-frommarkdownpreview:reset-zoom': () => {
    ...
    if (this.file) {
      ...
      this.disposables.add(
        this.editor.getBuffer().onDidStopChanging(function () {
          if (atom.config.get('iansasciidoc-preview-frommarkdownpreview.liveUpdate')) {
            changeHandler()
          }
        })
      )
    ...
    this.disposables.add(
      atom.config.onDidChange(
        'iansasciidoc-preview-frommarkdownpreview.breakOnSingleNewline',
        changeHandler
      )
    )

    this.disposables.add(
      atom.config.observe('iansasciidoc-preview-frommarkdownpreview.useGitHubStyle', useGitHubStyle => {
        if (useGitHubStyle) {
  ...
  getURI () {
    if (this.file != null) {
      return `iansasciidoc-preview-frommarkdownpreview://${this.getPath()}`
    } else {
      return `iansasciidoc-preview-frommarkdownpreview://editor/${this.editorId}`
    }
  }
...
//ianschange  Removed but this may be needed wi asciidoc style to render in preview???
  getIansasciidocPreviewFrommarkdownpreviewCSS () { // ianscomment called when saveAs not copyClipboard or Preview
    const iansasciidocPreviewFrommarkdownpreviewRules = []
    const ruleRegExp = /\.iansasciidoc-preview-frommarkdownpreview/
    const cssUrlRegExp = /url\(atom:\/\/iansasciidoc-preview-frommarkdownpreview\/assets\/(.*)\)/
      ...
      if (stylesheet.rules != null) {
        for (const rule of stylesheet.rules) {
          ...
          if (rule.selectorText && rule.selectorText.match(ruleRegExp)) {
            iansasciidocPreviewFrommarkdownpreviewRules.push(rule.cssText)
          }
        }
      }
    }

    return iansasciidocPreviewFrommarkdownpreviewRules
  ...
  } //END getIansasciidocPreviewFrommarkdownpreviewCSS
...
<html>
  <head>
      <meta charset="utf-8" />
      <title>${title}</title>\n` +
      //`<style>${this.getIansasciidocPreviewFrommarkdownpreviewCSS()}</style>` +  // getIansasciidocPreviewFrommarkdownpreviewCSS used only in saveAs HTML
...
`  </head> \n` +
//  <body class='iansasciidoc-preview-frommarkdownpreview' data-use-github-style>${htmlBody}</body> //ORIGINAL
//`  <body class='iansasciidoc-preview-frommarkdownpreview' >` +
`  <body >\n` +
`${htmlBody}
  </body>
...
<head>
    <meta charset="utf-8" />
    <title>${title}</title>
    <style>${this.getIansasciidocPreviewFrommarkdownpreviewCSS()}</style>
</head>
